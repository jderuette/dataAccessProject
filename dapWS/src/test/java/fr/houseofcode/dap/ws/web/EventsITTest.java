...
public class EventsITTest {
    ...
    public void testNextEvent() throws Exception {
        ...
        BDDMockito.given(service.display("lydie", service.getNextEvent("lydie"))).willReturn("cours");

        mvc.perform(MockMvcRequestBuilders.get("/events/next").contentType(MediaType.TEXT_PLAIN_VALUE).param("userId",
                "lydie")).andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().string("cours"));
    }
     ...
     */
    @Test
    public void testNextEventCorrection() throws Exception {
